...
for (i in 1:length(snakemake@input)) {
  if (grepl("mother", snakemake@input[[i]])) {
    if (grepl("resources/forfigs/gd", snakemake@input[[i]])) {
...
create_facet_plot = function(data, genome_val, outcome_val,x_label) {
  # Subset - for the specific genome and outcome combination
  subset_data = subset(data, genome == genome_val & outcome == outcome_val)
  
  # Equal x and y limits
  overall_range = range(
    c(subset_data$clmin1, subset_data$clmax1, subset_data$Estimate1, subset_data$clmin2, subset_data$clmax2, subset_data$Estimate2), 
    na.rm = TRUE
  )
  max_abs_range = max(abs(overall_range))
  final_range = c(-max_abs_range, max_abs_range)
  
  correlation = round(cor(subset_data$Estimate1, subset_data$Estimate2, method = 'pearson', use="complete.obs"),digits =2)
  
  p1 = ggplot(subset_data, aes(x=Estimate1, y=Estimate2)) +
    geom_abline(slope = 1,intercept =0, color = "grey40", linetype = "dashed") +
    geom_errorbar(aes(ymin = clmin1, ymax = clmax1),width =0) +
