## Interaction effect from both maternal and fetal
...
parsed_opts <- list(
  ...
  make_option("--prs",
              ...
              default = NA,
              type = 'character'),
  make_option("--fetal_ID",
              action = "store",
              default = NA,
              type = 'character'),
  make_option("--fetal_prs",
              action = "store",
              default = NA,
              type = 'character'),
  make_option("--genome",
...
if (interactive()) {
  opt <- list(ID='results/replication/gd_Mother_parityall/ID.tsv',
...
## input for fetal
fetal_ID <- fread(opt$fetal_ID)
fetal_prs <- fread(opt$fetal_prs) %>%
