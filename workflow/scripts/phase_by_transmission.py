...
if snakemake.wildcards.motherorchild == "child" and \
    snakemake.wildcards.parity == "parity1" and \
    snakemake.wildcards.pheno == "gd":
        pp = pd.DataFrame()
        pp.to_csv(snakemake.output[0], index=False)
        pp.to_csv(snakemake.output[1], index=False)
        pp.to_csv(snakemake.output[2], index=False)
